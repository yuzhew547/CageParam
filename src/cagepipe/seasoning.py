...
# Tools are resolved from the user's PATH (so any conda/AmberTools env that is
# active will be picked up). OpenBabel's own BABEL_LIBDIR / BABEL_DATADIR env
# vars are still honored for its data directories.
...
def xyz_to_pdb(xyz_path, pdb_path):
    obabel = _find_exe(OBABEL_NAMES)
    if obabel is None:
        raise RuntimeError(
            "OpenBabel (obabel) not found on PATH. Install OpenBabel or add it "
            "to your PATH."
        )
...
def run_autoparam(pdb_path, stem, charge, resname, atom_type="gaff2"):
    ...
    antechamber = _find_exe(ANTECHAMBER_NAMES)
    if antechamber is None:
        raise RuntimeError(
            "antechamber not found on PATH. Activate AmberTools (e.g. "
            "`conda activate AmberTools`)."
        )
    parmchk2 = _find_exe(PARMCHK2_NAMES)
    if parmchk2 is None:
        raise RuntimeError(
            "parmchk2 not found on PATH. Activate AmberTools (e.g. "
            "`conda activate AmberTools`)."
        )
